...
local src2 = game:HttpGet(base .. "modules/606849621/extras.lua")
local fn2 = src2 and loadstring(src2)
if fn2 then
	local ok, err = pcall(function() fn2()(Tabs) end)
	if not ok then warn("[Carbon] extras.lua error: " .. tostring(err)) end
else
	warn("[Carbon] extras.lua failed to fetch or compile")
end
