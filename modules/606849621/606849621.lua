...
return function(Tabs)
	...
	-- ── No Tire Pop ───────────────────────────────────────────────────
	-- u91.AreTiresPopped drives the speed penalty + forces nitro off.
	-- We hold it false every heartbeat while we have a vehicle packet ref.
	local tirePopConn
		...
		Callback = function(enabled)
			if enabled then
				tirePopConn = RunService.Heartbeat:Connect(function()
					local u91 = findUpvalue(function(v)
						return type(v) == "table" and v.AreTiresPopped ~= nil and v.Make ~= nil
					end)
					if u91 then u91.AreTiresPopped = false end
				end)
			else
