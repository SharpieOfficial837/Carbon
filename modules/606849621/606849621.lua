...
-- Robbery name/state lookup
local ROBBERY_NAMES = {
	"Bank", "Jewelry Store", "Museum", "Power Plant", "Passenger Train",
	"Cargo Train", "Cargo Ship", "Cargo Plane", "Gas Station", "Donut Store",
	"Grocery Store", "Money Truck", "Home Vault", "Tomb", "Crown Jewel",
	"Mansion", "Oil Rig"
}
local STATE_COLORS = {
	[1] = Color3.fromRGB(80, 220, 80),   -- OPENED
	[2] = Color3.fromRGB(220, 180, 0),   -- STARTED
	[3] = Color3.fromRGB(180, 50, 50),   -- CLOSED
}
...
return function(Tabs)
	...
	-- ── No Tire Pop ───────────────────────────────────────────────────
	-- The game reads VehicleTiresLastPop (timestamp) and VehicleTireHealth
	-- attributes on the vehicle model each frame to compute AreTiresPopped.
	-- We reset both to "healthy" every heartbeat via VehicleUtils.
	local tirePopConn
	local VehicleUtils = require(game:GetService("ReplicatedStorage"):FindFirstChild("VehicleUtils", true))
	PlayerSection:AddToggle('NoTirePop', {
		Text = 'No Tire Pop',
		Default = false,
		Callback = function(enabled)
			if enabled then
				tirePopConn = RunService.Heartbeat:Connect(function()
					local model = VehicleUtils.GetLocalVehicleModel()
					if not model then return end
					local maxHp = model:GetAttribute("MaxVehicleTireHealth")
					if maxHp then
						model:SetAttribute("VehicleTiresLastPop", 0)
						model:SetAttribute("VehicleTireHealth", maxHp)
					end
				end)
			else
				if tirePopConn then tirePopConn:Disconnect() tirePopConn = nil end
			end
		end
	})
